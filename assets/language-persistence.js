(() => {
  ...
  function installPortalButtons() {
    ...
    const style = document.createElement('style');
    style.textContent = `.button-92{--c:#fff;--button-color:${selected};display:inline-block;margin:18px 10px;padding:.1em .3em;border:none;color:#0000;background:linear-gradient(90deg,#0000 33%,#fff5,#0000 67%) var(--_p,100%)/300% no-repeat,var(--button-color);font:bold 2rem/1.25 "Lucida Console",Monaco,"Courier New",monospace;text-decoration:none;text-shadow:calc(var(--_i,-1)*.08em) -.01em 0 var(--c),calc(var(--_i,-1)*-.08em) .01em 2px #0004;cursor:pointer;transform:perspective(500px) rotateY(calc(20deg*var(--_i,-1)));outline-offset:.1em;transition:.3s}.button-92:hover,.button-92:focus-visible{--_p:0%;--_i:1}.button-92:active{color:var(--c);text-shadow:none;box-shadow:inset 0 0 0 999px #0005;transition:0s}@media(max-width:620px){.button-92{font-size:1.45rem;margin:14px 6px}}`;
    document.head.append(style);
    links.forEach((link) => {
  ...
  function installCurrentConditions() {
    ...
    const conditions = document.querySelector('.phases');
    const mantra = document.getElementById('mantra');
    const frequency = document.getElementById('frequencyLabel')?.parentElement;
    const languageButton = document.getElementById('languageSwitch');
    if (!heading || !conditions) return;

    const style = document.createElement('style');
    style.textContent = `.current-conditions{max-width:none;margin:4px auto;text-align:center;font-size:18px;line-height:1.35}.current-condition-line{margin:1px 0}.current-condition-name{color:#54ff3d}.current-condition-value{color:#fff}`;
    document.head.append(style);
    ...
    const render = () => {
      ...
      conditions.innerHTML = `
        <div class="current-condition-line"><span class="current-condition-name">OBJECT OF DESIRE</span> :: <span class="current-condition-value">BALENCIAGA KNIGHT BOOTS</span></div>
        <div class="current-condition-line"><span class="current-condition-name">MOST IMPORTANT IMAGE</span> :: <span class="current-condition-value">GOLD CHAIN</span></div>
        <div class="current-condition-line"><span class="current-condition-name">MARKET MOOD</span> :: <span class="current-condition-value">PRIVACY</span></div>
        <div class="current-condition-line"><span class="current-condition-name">INTERNET WEATHER</span> :: <span class="current-condition-value">HUMID</span></div>
        <div class="current-condition-line"><span class="current-condition-name">JAMES SALAMANDER STATUS</span> :: <span class="current-condition-value">ON ASSIGNMENT</span></div>`;
