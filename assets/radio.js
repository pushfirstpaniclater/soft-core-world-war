...
player.innerHTML=`<div class="scww-radio-title">SCWW RADIO</div><div class="scww-radio-screen"><div>NOW PLAYING:</div><div class="scww-radio-track"></div><div class="scww-radio-count"></div><div class="scww-radio-time"><span class="elapsed">0:00</span><span class="duration">∞</span></div><div class="scww-radio-meter"><span></span></div><div class="status">SIGNAL: DORMANT</div><div class="scww-radio-resume">PRESS PLAY TO RESUME</div></div><div class="scww-radio-transport"><button class="prev">◀</button><button class="play">▶</button><button class="next">▶▶</button></div><div class="scww-radio-controls"><button class="stop">■ STOP</button><button class="loop">↻ OFF</button></div><div class="scww-radio-volume-label"><span>VOLUME</span><span class="volume-value">27%</span></div><input class="scww-radio-volume" type="range" min="0" max="1" step="0.01" value="0.27" aria-label="Radio volume"><div class="scww-radio-hint">SITE-WIDE PORTAL PLAYLIST</div><audio preload="metadata"></audio>`;
...
state.current=Math.max(0,Math.min(tracks.length-1,Number(state.current)||0));state.volume=Math.max(0,Math.min(1,Number(state.volume)||0));state.loop=Boolean(state.loop);
...
function paint(){title.textContent=tracks[state.current].title;count.textContent=String(state.current+1).padStart(2,'0')+' / '+String(tracks.length).padStart(2,'0');loopBtn.textContent=state.loop?'↻ ON':'↻ OFF';loopBtn.classList.toggle('active',state.loop);loopBtn.setAttribute('aria-pressed',String(state.loop));audio.loop=state.loop;vol.value=state.volume;volText.textContent=Math.round(state.volume*100)+'%';elapsed.textContent=fmt(state.time);duration.textContent=tracks[state.current].type==='portal'?'∞':'--:--'}
...
function stopMedia(reset=false){stopDrone();audio.pause();clearInterval(meterTimer);meterTimer=null;meter.style.width='5%';if(reset){state.time=0;droneBase=0;try{audio.currentTime=0}catch{}}}
...
vol.addEventListener('input',setVolume);playBtn.addEventListener('click',()=>isPlaying?pause():start(false));q('.stop').addEventListener('click',()=>{stopMedia(true);isPlaying=false;state.playing=false;state.time=0;playBtn.textContent='▶';status.textContent='SIGNAL: DORMANT';elapsed.textContent='0:00';save()});q('.prev').addEventListener('click',()=>change(-1));q('.next').addEventListener('click',()=>change(1));loopBtn.addEventListener('click',()=>{state.loop=!state.loop;audio.loop=state.loop;paint();save()});audio.addEventListener('loadedmetadata',()=>duration.textContent=fmt(audio.duration));audio.addEventListener('timeupdate',()=>{state.time=audio.currentTime;elapsed.textContent=fmt(audio.currentTime);if(Number.isFinite(audio.duration))duration.textContent=fmt(audio.duration)});audio.addEventListener('ended',()=>{if(state.loop)return;if(state.current<tracks.length-1){state.current+=1;state.time=0;start(false)}else{isPlaying=false;state.playing=false;playBtn.textContent='▶';status.textContent='SIGNAL: COMPLETE';save()}});
